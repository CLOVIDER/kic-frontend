...
export default function BlockNoteViewer({
  ...
  const editor = useCreateBlockNote({
    ...
    uploadFile: enableImageUpload ? handleUpload : undefined, // 이미지 업로드 핸들러 설정
  })
  ...
  useEffect(() => {
    if (editor) {
      ...
      if (domElement) {
        domElement.setAttribute('contenteditable', editable ? 'true' : 'false')
      }
    }
  }, [editor, editable])
  ...
  return (
    <div
      className={`flex justify-start w-full ${editable ? '' : 'pointer-events-none'}`}
    >
